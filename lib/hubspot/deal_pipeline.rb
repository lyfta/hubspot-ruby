  ...
  class DealPipeline
    ...
    class << self
      def find(pipeline_id)
        response = OldHubspot::Connection.get_json(PIPELINE_PATH, { pipeline_id: pipeline_id })
        new(response)
      end

      def all
        response = OldHubspot::Connection.get_json(PIPELINES_PATH, {})
        response.map { |p| new(p) }
      end

      # Creates a DealPipeline
      # {https://developers.hubspot.com/docs/methods/deal-pipelines/create-deal-pipeline}
      # @return [OldHubspot::PipeLine] Company record
      def create!(post_data={})
        response = OldHubspot::Connection.post_json(PIPELINES_PATH, params: {}, body: post_data)
        new(response)
      end
    end
    ...
    def destroy!
      OldHubspot::Connection.delete_json(PIPELINE_PATH, pipeline_id: @pipeline_id)
    end

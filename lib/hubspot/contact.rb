class OldHubspot::Contact < OldHubspot::Resource
  self.id_field = "vid"
  ...
  class << self
    def all(opts = {})
      OldHubspot::PagedCollection.new(opts) do |options, offset, limit|
        response = OldHubspot::Connection.get_json(
          ALL_PATH,
    ...
    def find_by_email(email)
      response = OldHubspot::Connection.get_json(FIND_BY_EMAIL_PATH, email: email)
      from_result(response)
    end

    def find_by_user_token(token)
      response = OldHubspot::Connection.get_json(FIND_BY_USER_TOKEN_PATH, token: token)
      from_result(response)
    end
    ...
    def create_or_update(email, properties = {})
      request = {
        properties: OldHubspot::Utils.hash_to_properties(properties.stringify_keys, key_name: "property")
      }
      response = OldHubspot::Connection.post_json(CREATE_OR_UPDATE_PATH, params: { email: email}, body: request)
      from_result(response)
    end

RSpec.describe OldHubspot::DealPipeline do
  ...
  describe ".find" do
    it "retrieves a record by id" do
      VCR.use_cassette("find_deal_pipeline") do
        deal_pipeline = OldHubspot::DealPipeline.create!(label: "New Pipeline")
  ...
  describe ".create!" do
    it "creates a new record" do
      VCR.use_cassette("create_deal_pipeline") do
        result = OldHubspot::DealPipeline.create!(label: "New Pipeline")
  ...
  def hubspot_api_url(path)
    URI.join(OldHubspot::Config.base_url, path, "?hapikey=demo")
  end
end

describe OldHubspot::Deal do
  ...
  describe "#initialize" do
    ...
    it  { should be_an_instance_of OldHubspot::Deal }
  ...
  describe '.find_by_company' do
    ...
    let(:company) { OldHubspot::Company.create!('Test Company') }
    let(:deal) { OldHubspot::Deal.create!(portal_id, [company.vid], [vid], { amount: amount }) }
  ...
  describe '.recent' do
    ...
    it 'must get the recents updated deals' do
      ...
      expect(last).to be_a OldHubspot::Deal
    ...
    it 'it must offset the deals' do
      deal = OldHubspot::Deal.recent(count: 1, offset: 1).first
  ...
  describe '#destroy!' do
    ...
    it 'should remove from hubspot' do
      pending
      expect(OldHubspot::Deal.find(deal.deal_id)).to_not be_nil

      expect(deal.destroy!).to be_true
      expect(deal.destroyed?).to be_true

      expect(OldHubspot::Deal.find(deal.deal_id)).to be_nil
    end
  end

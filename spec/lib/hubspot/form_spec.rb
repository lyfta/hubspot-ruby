describe OldHubspot::Form do
  ...
  let(:example_form_hash) do
    VCR.use_cassette('form_example') do
      HTTParty.get("https://api.hubapi.com#{OldHubspot::Form::FORMS_PATH}/#{guid}/?hapikey=demo").parsed_response
    end
  end

  describe '#initialize' do
    subject { OldHubspot::Form.new(example_form_hash) }
  ...
  describe '.all' do
    ...
    it 'returns all forms' do
      forms = OldHubspot::Form.all
      expect(forms.count).to be > 20

      form = forms.first
      expect(form).to be_a(OldHubspot::Form)
    end
  end

  describe '.find' do
    cassette 'form_find'
    subject { OldHubspot::Form.find(guid) }
    ...
    context 'when the form is not found' do
      it 'raises an error' do
        expect { OldHubspot::Form.find(-1) }.to raise_error(OldHubspot::RequestError)
      end
    end
  end

  describe '.create' do
    subject { OldHubspot::Form.create!(params) }
    ...
    context 'without all required parameters' do
      ...
      it 'raises an error' do
        expect { OldHubspot::Form.create!({}) }.to raise_error(OldHubspot::RequestError)
      end
    end
  end
  ...
  describe '#fields' do
    context 'returning all the fields' do
      ...
      let(:form) { OldHubspot::Form.new(example_form_hash) }
  ...
  describe '#submit' do
    ...
    let(:form) { OldHubspot::Form.find('561d9ce9-bb4c-45b4-8e32-21cdeaa3a7f0') }

    context 'with a valid portal id' do
      before do
        OldHubspot.configure(hapikey: 'demo', portal_id: '62515')
      end
    ...
    context 'with an invalid portal id' do
      before do
        OldHubspot.configure(hapikey: 'demo', portal_id: 'xxxx')
      end
    ...
    context 'when initializing Hubspot::Form directly' do
      let(:form) { OldHubspot::Form.new('guid' => '561d9ce9-bb4c-45b4-8e32-21cdeaa3a7f0') }
  ...
  describe '#destroy!' do
    ...
    let(:form) { OldHubspot::Form.find('beb92950-ca65-4daf-87ae-a42c054e429f') }

describe OldHubspot::Form do
  ...
  describe '.find' do
    ...
    context 'when the form is not found' do
      it 'raises an error' do
        expect { OldHubspot::Form.find(-1) }.to raise_error(OldHubspot::NotFoundError)
      end
    end
  end

  describe '.create' do
    subject { OldHubspot::Form.create!(create_params) }
  ...
  describe '#submit' do
    ...
    let(:form) { OldHubspot::Form.all.first }
    ...
    context 'with an invalid portal id' do
      before do
        OldHubspot.configure(access_token: ENV.fetch("HUBSPOT_ACCESS_TOKEN"), portal_id: "xxx")
      end
    ...
    context 'when initializing Hubspot::Form directly' do
      let(:f) { OldHubspot::Form.new('guid' => form.guid) }
  ...
  describe '#update!' do
    ...
    let(:form) { OldHubspot::Form.create!(create_params) }

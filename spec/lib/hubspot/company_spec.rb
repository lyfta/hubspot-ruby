describe OldHubspot::Contact do
  ...
  describe "#initialize" do
    subject{ OldHubspot::Company.new(example_company_hash) }
    it{ should be_an_instance_of OldHubspot::Company }
  ...
  describe ".create!" do
    ...
    let(:params){{}}
    subject{ OldHubspot::Company.create!(name, params) }
  ...
  describe ".add_contact!" do
    ...
    let(:company){ OldHubspot::Company.create!("company_#{Time.now.to_i}@example.com") }
    let(:contact){ OldHubspot::Contact.create!("contact_#{Time.now.to_i}@example.com") }
    subject { OldHubspot::Company.find_by_id(company.vid) }

    before { OldHubspot::Company.add_contact! company.vid, contact.vid }
  ...
  describe ".find_by_id" do
    context 'given an uniq id' do
      ...
      subject{ OldHubspot::Company.find_by_id(vid) }
      ...
      context "when the contact cannot be found" do
        it 'raises an error' do
          expect { OldHubspot::Company.find_by_id(9999999) }.to raise_error(OldHubspot::RequestError)
        end
      end
    end
  end
  ...
  describe ".find_by_domain" do
    context 'given a domain' do
      ...
      subject(:companies) { OldHubspot::Company.find_by_domain("example.com") }
      ...
      context "when a company is found" do
        ...
        it 'must contain all available properties' do
          companies[0..9].each do |company|
            expect(company.properties).to eql OldHubspot::Company.find_by_id(company.vid).properties
          end
        end
      end

      context "when a company cannot be found" do
        subject { OldHubspot::Company.find_by_domain("asdf1234baddomain.com") }
        it { should be_an_instance_of Array }
        it { should be_empty }
      end
    end
    ...
    context 'given a domain and parameters' do
      ...
      subject(:companies) { OldHubspot::Company.find_by_domain("example.com", limit: 2, properties: ["name", "createdate"], offset_company_id: 117004411) }
  ...
  describe '.all' do
    context 'all companies' do
      ...
      it 'must get the companies list' do
        companies = OldHubspot::Company.all
      ...
      context 'all_with_offset' do
        ...
        it 'must filter only 2 companies' do
          response = OldHubspot::Company.all_with_offset(count: 2)
    ...
    context 'recent companies' do
      ...
      it 'must get the companies list' do
        companies = OldHubspot::Company.all(recently_updated: true)
  ...
  describe "#update!" do
    ...
    let(:company){ OldHubspot::Company.new(example_company_hash) }
    ...
    context "when the request is not successful" do
      let(:company){ OldHubspot::Company.new({ "vid" => "invalid", "properties" => {}})}
      it "raises an error" do
  ...
  describe "#batch_update!" do
    ...
    context 'update via vid' do
      ...
      it 'should update companies' do
        OldHubspot::Company.batch_update!(updated_companies)
        checked_company = OldHubspot::Company.find_by_id(company.vid)
    ...
    context 'missing vid or objectId' do
      ...
      it 'should raise error with expected message' do
        expect { OldHubspot::Company.batch_update!(updated_companies) }.to raise_error(OldHubspot::InvalidParams, 'expecting vid or objectId for company')
      end
    end
  end
  ...
  describe "#destroy!" do
    ...
    let(:company){ OldHubspot::Company.create!("newcompany_y_#{Time.now.to_i}@hsgem.com") }
  ...
  describe "#contacts" do
    let(:company){ OldHubspot::Company.new(company_with_contacts_hash) }
    subject do

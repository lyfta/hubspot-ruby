describe OldHubspot::ContactList do
  ...
  let(:static_list) { OldHubspot::ContactList.all(static: true, count: 3).last }
  let(:dynamic_list) { OldHubspot::ContactList.all(dynamic: true, count: 1).first }
  ...
  describe '#contacts' do
    ...
    expect_count_and_offset do |params|
      OldHubspot::ContactList.find(1).contacts(params)
    end
  end
  ...
  describe '.all' do
    context 'all list types' do
      ...
      expect_count_and_offset { |params| OldHubspot::ContactList.all(params) }
    end
  ...
  describe '.find' do
    context 'given an id' do
      ...
      context 'Wrong parameter type given' do
        it 'raises an error' do
          expect { OldHubspot::ContactList.find(static_list) }.to raise_error(OldHubspot::InvalidParams)
        end
      end

      context 'when the contact list is not found' do
        it 'raises an error' do
          expect { OldHubspot::ContactList.find(-1) }.to raise_error(OldHubspot::RequestError)
        end
      end
    end
    ...
    context 'given a list of ids' do
      ...
      it 'find lists of contacts' do
        lists = OldHubspot::ContactList.find([2, 3, 4])
        list = lists.first
        expect(list).to be_a(OldHubspot::ContactList)
  ...
  describe '#add' do
    ...
    context 'static list' do
      it 'returns true if contacts have been added to the list' do
        contact = OldHubspot::Contact.all(count: 1).first
        mock(OldHubspot::Connection).post_json("/contacts/v1/lists/:list_id/add", { :params=>{ :list_id=>4}, :body=>{ :vids=>[contact.vid]}}) { { 'updated' => [contact.vid] } }
    ...
    context 'dynamic list' do
      it 'raises error if try to add a contact to a dynamic list' do
        contact = OldHubspot::Contact.new(example_contact_hash)
        expect { dynamic_list.add(contact) }.to raise_error(OldHubspot::RequestError)
      end
    end
  end
  ...
  describe '#remove' do
    ...
    context 'static list' do
      ...
      it 'returns false if the contact cannot be removed' do
        contact_not_present_in_list = OldHubspot::Contact.new(example_contact_hash)
    ...
    context 'dynamic list' do
      it 'raises error if try to remove a contact from a dynamic list' do
        ...
        expect { dynamic_list.remove(contact) }.to raise_error(OldHubspot::RequestError)
      end
    end
  end
  ...
  describe '#update!' do
    ...
    let(:contact_list) { OldHubspot::ContactList.new(example_contact_list_hash) }
  ...
  describe '#destroy!' do
    ...
    let(:contact_list) { OldHubspot::ContactList.create!({ name: "newcontactlist_#{Time.now.to_i}"}) }
